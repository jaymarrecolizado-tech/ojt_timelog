...
return new class extends Migration
{
    ...
    {
        // Generate cryptographically secure location secret
        $locationSecret = hash('sha256', Str::random(64) . time() . env('APP_KEY'));
        
        // Insert default location with secure secret
        DB::table('locations')->insert([
            'id' => Str::uuid(),
            'name' => 'Main Gate',
            'description' => 'Primary entrance/exit point',
            'secret_key' => $locationSecret,
            'is_active' => true,
            'created_at' => now(),
            'updated_at' => now(),
        ]);

        // Only create default admin if environment variables are set (production safety)
        $adminEmail = env('DEFAULT_ADMIN_EMAIL');
        $adminPassword = env('DEFAULT_ADMIN_PASSWORD');
        
        if ($adminEmail && $adminPassword && app()->environment('local', 'development')) {
            $adminId = Str::uuid();
    ...
    public function down(): void
    {
        ...
        DB::table('users')->where('email', env('DEFAULT_ADMIN_EMAIL'))->delete();
